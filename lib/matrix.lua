...
function matrix:new( rows, columns, value )
	...
	-- build new matrix
		...
		for i = 1,rows do
			mtx[i] = {}
			for j = 1,columns do
				mtx[i][j] = (type(value) ~= "function" and value) or value()
			end
		end
	end
...
function matrix.mul( m1, m2 )
	...
	local mtx = {}
	assert(#m1[1]==#m2, "1st columns~= 2nd rows ("..#m1.."x"..#m1[1]..","..#m2.."x"..#m2[1]..")") -- comment out to skip check
	for i = 1,#m1 do
		mtx[i] = {}
		for j = 1,#m2[1] do
...
function matrix.ebem( m1,m2 ) -- element by element multiplication
	assert(#m1 == #m2 and #m1[1] == #m2[1], "matrix size mismatch")
	local mtx = {}
	for i = 1,#m1 do
		mtx[i] = {}
		for j = 1,#m1[1] do
...
function matrix.ebed( m1,m2 ) -- element by element division
	assert(#m1 == #m2 and #m1[1] == #m2[1], "matrix size mismatch")
	local mtx = {}
	for i = 1,#m1 do
		mtx[i] = {}
...
function matrix.ebef( m1,m2,f,... ) -- element by element custom function
	local mtx = {}
	if type(m2) == "table" then
		assert(#m1 == #m2 and #m1[1] == #m2[1], "matrix size mismatch")
		assert(type(f)=="function", "No function provided")
		for i = 1,#m1 do
			mtx[i] = {}
			for j = 1,#m1[1] do
				mtx[i][j] = f(m1[i][j], m2[i][j], ...)
			end
		end
	elseif type(m2) == "function" then
		for i=1,#m1 do
			mtx[i] = {}
			for j=1,#m1[1] do
				mtx[i][j] = m2(m1[i][j], j, ...)
...
function matrix.todiag(m1) -- converts vector to diagonal matrix
	assert(#m1==1 or #m1[1]==1, "Not a vector")
	local mtx = {}
	if #m1==1 then m1 = m1[1] end
	for i=1,#m1,1 do
		mtx[i] = {}
		for j=1,#m1,1 do
			if i==j then
...
function matrix.getdiag(m1) -- converts vector to diagonal matrix
	assert(#m1==#m1[1], "Not square matrix")
	local vec = {}
	for i=1,#m1,1 do
		vec[i] = m1[i][i]
	end
	return vec --setmetatable( vec, matrix_meta )
end

function matrix.toscalar(m1)
	assert(#m1==1 and #m1[1]==1, "Not 1x1 matrix")
	return m1[1][1]
end
--////////////////////////--
--// METATABLE HANDLING //--
